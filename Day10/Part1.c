...
static int inline continue_trail(uint32_t looking_for, uint32_t* map, char* visited_map, int x, int y, const int width, const int height, uint64_t* depth){
    (*depth)++;
    ...
    if (looking_for == 10){
        ...
        visited_map[get_pos(x, y, width)] = VISITED;
        return 0;
    }
...
static uint64_t inline count_visited(char* visited_map, int width, int height){
    uint64_t count = 0;
    for (int i=0;i<width*height;i++){
        if (visited_map[i] == VISITED){
            count++;
        }
    }
    return count;
...
static int inline start_trailhead(uint32_t* map, int x, int y, int width, int height, uint64_t* result){
    uint64_t depth = 0;
    char* visited_map = (char*)calloc(width*height, sizeof(char));
    if (!visited_map){
        return MALLOCERROR;
    }
