...
static void inline count_antinodes_per_pair(int x, int y, int x1, int y1, int width, int height, enum Token* antinodes_map){
    ...
    if (dx != 0){
        ...
        for(int i = 0; i<width; i++){
            ...
            // check that liney is integer
            if (fabs(liney - round(liney)) <= DIFFTOLL){
                if ((int)round(liney) < 0 || (int)round(liney) >= height){
                    continue;
                }
                antinodes_map[get_pos(linex , (int)round(liney), width)] = ANTINODE;
            }
        }

    } else  { // vertical line
        for(int i =0; i<height; i++){
            antinodes_map[get_pos(x, i, width)] = ANTINODE;
        }
    }
